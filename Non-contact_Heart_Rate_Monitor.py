# -*- coding: utf-8 -*-
"""
Created on Thu Aug 22 14:01:08 2019
...
# Constants
WINDOW_TITLE = 'Non-contact Heart Rate  Monitor.'
...
MIN_HZ = 0.6667       # 40 BPM - minimum allowed heart rate
MAX_HZ = 3.6667       # 220 BPM - maximum allowed heart rate
MIN_FRAMES = 200    # Minimum number of frames required before heart rate is computed.  Higher values are slower, but more accurate.
DEBUG_MODE = True
...
def sliding_window_demean(signal_values, num_windows):
    ...
    for i in xrange(0, len(signal_values), window_size):
...
def draw_graph(signal_values, graph_width, graph_height):
    ...
    for i in xrange(0, len(signal_values) - 1):
...
def filter_signal_data(values, fps):
    ...
    demeaned = sliding_window_demean(detrended, 12)
...
def run_pulse_observer(detector, predictor, webcam, window):
    ...
    while cv2.getWindowProperty(window, 0) == 0:
        ...
        if graph_width == 0:
            graph_width = int(view.shape[1] * 0.5)
            if DEBUG_MODE:
